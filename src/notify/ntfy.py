...
def send_ntfy(
    ...
    msg_lines = [
        ...
        f"- 🎛️ **Audio:** {fields['audio_summary']}" if fields["audio_summary"] else None,
        f"- 📈 **Torrent:** {fields['torrent_health']}" if fields["torrent_health"] else None,
        f"- 🎟️ **Access:** {fields['freeleech_label']}" if fields["freeleech_label"] else None,
        f"- 🕒 **Added:** {fields['added_date']}" if fields["added_date"] else None,
        f"- 🆔 **ID:** {fields['asin'] or fields['isbn']}" if (fields["asin"] or fields["isbn"]) else None,
        " ---\n",
